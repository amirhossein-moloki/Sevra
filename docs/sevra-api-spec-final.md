...
## 9) ترتیب پیشنهادی برای پیاده‌سازی (Implementation Priority)

برای اطمینان از توسعه‌ی ساختاریافته و مدیریت وابستگی‌ها، پیاده‌سازی Endpointها بر اساس اولویت زیر پیشنهاد می‌شود. این ترتیب به تیم اجازه می‌دهد تا ابتدا زیرساخت‌ها و قابلیت‌های اصلی را بسازد و سپس به سراغ ویژگی‌های تکمیلی برود.

| اولویت (Priority) | ماژول (Module) | Endpointهای کلیدی (Key Endpoints) | ملاحظات (Notes) |
|---|---|---|---|
| 1 | **زیرساخت (Infra)** | `errorHandler`, `auth middleware`, `validation`, `response envelope` | **پیش‌نیاز مطلق.** این بخش باید قبل از هر چیز دیگری آماده شود. |
| 2 | **احراز هویت و آنبوردینگ (Auth & Onboarding)** | `/auth/login`, `/auth/me`, `/onboarding/salons` | هسته‌ی ورود کاربر و ایجاد اولین سالن. بدون این بخش، پنل قابل استفاده نیست. |
| 3 | **مدیریت پایه سالن (Core Salon Mgmt)** | `/salons/{id}`, `/users`, `/services`, `/shifts` | CRUD کامل برای مدیریت پرسنل، سرویس‌ها و تنظیمات اولیه سالن. |
| 4 | **هسته رزرو (Booking Core)** | `/availability/slots`, `/bookings` (Panel) | پیاده‌سازی منطق اصلی محاسبه زمان‌های در دسترس و ایجاد و مدیریت رزرو از طریق پنل. |
| 5 | **جریان رزرو عمومی (Public Booking Flow)** | `/public/.../services`, `/public/.../availability`, `/public/.../bookings` | فعال‌سازی قابلیت رزرو آنلاین برای مشتریان نهایی. |
| 6 | **عملیات پس از رزرو (Post-Booking)** | `/bookings/{id}/payments`, `/customers` | مدیریت پرداخت‌ها و پروفایل مشتریان. این بخش به تکمیل چرخه رزرو کمک می‌کند. |
| 7 | **ویژگی‌های ثانویه (Secondary Features)** | `/reviews`, `/cms`, `/commission-policy` | ماژول‌های تکمیلی که پس از پایداری کامل سیستم اصلی می‌توانند اضافه شوند. |
