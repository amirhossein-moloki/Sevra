...
import { User, Salon, UserRole } from '@prisma/client';
...
describe('Negative Permission (RBAC) E2E Tests', () => {
  ...
  beforeAll(async () => {
    ...
    managerToken = generateToken({ id: manager.id, salonId: salon.id, role: manager.role });
    receptionistToken = generateToken({ id: receptionist.id, salonId: salon.id, role: receptionist.role });
    staffToken = generateToken({ id: staff.id, salonId: salon.id, role: staff.role });
  });
